...
public class StudentController {
    ...
    private Map<Long, String> buildScheduleMap(List<Class> classes) {
        Map<Long, String> map = new HashMap<>();
        for (Class c : classes) {
            // Lịch học sẽ được lấy từ Schedule service, không có trong Class model
            map.put(c.getId(), "");
        }
        return map;
    }
    ...
    private Map<Long, String> buildRoomMap(List<Class> classes) {
        Map<Long, String> map = new HashMap<>();
        for (Class c : classes) {
            // Phòng học sẽ được lấy từ Schedule service, không có trong Class model
            map.put(c.getId(), "");
        }
        return map;
    }
    ...
    private Map<Long, Integer> buildCapacityMap(List<Class> classes) {
        ...
        for (Class c : classes) {
            ...
            map.put(c.getId(), capacity != null ? capacity : 30); // mặc định 30
        }
        return map;
    }
