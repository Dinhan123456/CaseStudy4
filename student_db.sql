-- =========================================
-- RESET & CREATE DATABASE (tuỳ chọn reset)
-- =========================================
-- DROP DATABASE IF EXISTS student_db;     -- ← Bỏ comment nếu muốn reset trắng
...
-- users: tài khoản đăng nhập cho ADMIN/TEACHER/STUDENT
...
-- students: 1-1 với users(id) có role=STUDENT, class_id là "lớp chủ quản"
...
-- teacher_class: giáo viên phụ trách một lớp
...
-- class_subject: gán GV dạy môn cho lớp
...
-- student_class: enrollment nhiều-nhiều
...
-- grades: theo student + subject + class + kỳ
...
-- (tuỳ chọn) timetables: nếu vẫn muốn giữ
...
-- Grades / Registrations / Student_class
...
CREATE INDEX idx_sc_class_student      ON student_class(class_id, student_id);


SET FOREIGN_KEY_CHECKS = 1;

-- =========================================
-- PATCH HỢP NHẤT (idempotent)
-- =========================================
USE student_db;
DELIMITER $$
...
END IF;
  ...
  -- schedules.created_by -> BIGINT + FK (đúng tên: fk_sch_created_by)
  IF EXISTS (
      SELECT 1 FROM information_schema.columns
...
END IF;
  ...
  IF NOT EXISTS (
      SELECT 1 FROM information_schema.table_constraints
      WHERE constraint_schema = DATABASE() AND table_name = 'schedules' AND constraint_name = 'fk_sch_created_by'
  ) THEN
ALTER TABLE schedules
    ADD CONSTRAINT fk_sch_created_by FOREIGN KEY (created_by) REFERENCES users(id);
END IF;

  -- registration_periods.created_by -> BIGINT + FK (đúng tên: fk_reg_period_created_by)
  IF EXISTS (
      SELECT 1 FROM information_schema.columns
...
END IF;
  ...
  IF NOT EXISTS (
      SELECT 1 FROM information_schema.table_constraints
      WHERE constraint_schema = DATABASE() AND table_name = 'registration_periods' AND constraint_name = 'fk_reg_period_created_by'
  ) THEN
ALTER TABLE registration_periods
    ADD CONSTRAINT fk_reg_period_created_by FOREIGN KEY (created_by) REFERENCES users(id);
END IF;

  -- student_registrations: unique
  IF NOT EXISTS (
...
END IF;
  ...
  -- Index tối ưu cho schedules (idempotent)
  IF NOT EXISTS (
      SELECT 1 FROM information_schema.statistics
...
END IF;
  ...
  -- Index ngược cho student_class
  IF NOT EXISTS (
      SELECT 1 FROM information_schema.statistics
      WHERE table_schema = DATABASE() AND table_name = 'student_class' AND index_name = 'idx_sc_class_student'
  ) THEN
CREATE INDEX idx_sc_class_student ON student_class(class_id, student_id);
END IF;
