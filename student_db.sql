...
END IF;
  ...
  -- tên FK PHẢI trùng với phần CREATE: fk_sch_created_by
  IF NOT EXISTS (
      SELECT 1 FROM information_schema.table_constraints
...
END IF;
  ...
  -- tên FK PHẢI trùng với phần CREATE: fk_reg_period_created_by
  IF NOT EXISTS (
      SELECT 1 FROM information_schema.table_constraints
...
END IF;
  ...
  -- ===== Khuyến nghị thêm: index ngược cho tra cứu theo lớp =====
  IF NOT EXISTS (
      SELECT 1 FROM information_schema.statistics
